...
[tn x]=generate_avoiding_data(.2,100);


xn=x(:,1);
yn=x(:,2);

[tf x2]=generate_avoiding_data(.2,100);
...
save('avoid_vars.mat','KS','Kf','KSvec','KSvecopt','Kfvec','Kfvecopt');
...
title('Knox test random permutation')
xlabel('\kappa') 
%text(3,60,strcat("p=",num2str(prand)))
hold on
xline(KS,'--','LineWidth',2);
hold off
subplot(2,2,2);
...
title('Knox test SOP permutation')
%text(3,60,strcat("p=",num2str(popt)))
xlabel('\kappa') 
hold on
xline(KS,'--','LineWidth',2);
hold off
...
xlabel('r') 
ylabel('L(r)') 
legend('Data','Rand Permutation','Location','best')
subplot(2,2,4);
plot([1:kpts]*kcut/kpts,Kf,'k--','LineWidth',2);
hold on
...
xlabel('r') 
ylabel('L(r)') 
legend('Data','SOP Permutation','Location','best')


print(gcf,'avoid_knox_fig.png','-dpng','-r1000')